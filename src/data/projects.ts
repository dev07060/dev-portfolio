import { Project } from "@/types/project";
export const projects: Project[] = [
  {
    id: "law-info-engine",
    type: "web",
    title: "Swifty-law",
    subtitle: "인용 기반 한국 법령 검색 RAG 엔진",
    description:
      "공식 법령정보를 인용 가능한 단위로 정규화·색인하고 hybrid retrieval로 검색하는 사내 공통 엔진. LLM은 검색된 공식 chunk와 citation 안에서만 답변합니다.",
    implementationPoints: [
      "Bronze/Silver/Gold 3계층 레이크로 원본 API에서 청크·임베딩을 언제든 재생성 가능하게 설계.",
      "SBERT dense + Milvus BM25 sparse를 RRFRanker(60)로 융합하고 corpus 기반 질의 확장 적용.",
      "국가법령 Open API quota를 일일 한도·최소 간격·지수 백오프 재시도로 1급 시민으로 보호.",
      "golden_retrieval_v1(74 dev / 30 holdout) nDCG@5 / MRR@10 / Recall@10 회귀 게이트로 릴리스 차단.",
    ],
    cardPresentation: {
      variant: "architecture",
      thumbnailScreenIndex: 3,
      description:
        "Bronze/Silver/Gold 레이크, Milvus dense/sparse ranking, citation metadata, evaluation gate를 갖춘 hybrid retrieval 백엔드입니다.",
      evidenceBadges: [
        "FastAPI",
        "Milvus",
        "RRFRanker",
        "Bronze/Silver/Gold",
        "evaluation",
      ],
      highlight:
        "golden retrieval set 기준 nDCG@5 / MRR@10 / Recall@10으로 릴리스를 게이트했습니다.",
    },
    techStack: ["Python", "FastAPI", "PostgreSQL", "Milvus", "SBERT", "Docker"],
    color: "from-amber-500 to-orange-600",
    iconType: "scale",
    screens: [
      {
        id: "api-landing",
        imageAlt: "Swifty-law API 진입 화면",
        title: "API 진입 화면",
        desc: "law-api.swifty.kr 개발자 진입 화면 — endpoint 인덱스와 health 표면.",
        imagePath: "/images/law-info-engine/api-landing.png",
        scrollable: true,
      },
      {
        id: "search-ui",
        imageAlt: "Swifty-law 검색 화면",
        title: "검색 화면",
        desc: "실제 서비스 검색 화면 — 자연어 질의로 조문 단위 결과를 돌려줍니다.",
        imagePath: "/images/law-info-engine/search-ui.png",
        scrollable: true,
      },
      {
        id: "search-ui-full",
        imageAlt: "Swifty-law 검색 결과·인용",
        title: "검색 결과·인용",
        desc: "모든 결과에 법령명·조문경로·시행일·출처 URL이 함께 노출됩니다.",
        imagePath: "/images/law-info-engine/search-ui-full.png",
        scrollable: true,
      },
      {
        id: "architecture",
        imageAlt: "Swifty-law 시스템 구성",
        title: "시스템 구성",
        desc: "5계층 흐름 — 원천 수집, 데이터 정제, 색인·지식그래프, 분석, API 노출.",
        imagePath: "/images/law-info-engine/architecture.svg",
        scrollable: true,
      },
      {
        id: "api-flows",
        imageAlt: "Swifty-law 검색·분석 경로",
        title: "검색·분석 경로",
        desc: "단순 조문 검색 경로와 쟁점 분해 + 지식그래프 확장 근거 경로의 단계별 흐름.",
        imagePath: "/images/law-info-engine/api-flows.svg",
        scrollable: true,
      },
    ],
    links: [
      {
        label: "서비스 보기",
        url: "https://law-api.swifty.kr/search-info/",
      },
      {
        label: "API 문서",
        url: "https://law-api.swifty.kr/docs",
      },
    ],
  },
  {
    id: "easy-contract-viewer",
    type: "mobile",
    title: "Easy Contract Viewer",
    subtitle: "AI 기반 보험 약관 검토 앱",
    description:
      "보험 약관 PDF를 가져와 mobile_rag_engine으로 로컬 색인하고, 검토 우선순위를 정리하며, 원문 PDF 하이라이트와 선택형 AI 요약까지 연결하는 Flutter 앱입니다.",
    implementationPoints: [
      "pdfrx 기반 PDF 추출과 조항 인식 chunking을 구현하고, 원문 하이라이트를 위해 페이지 좌표를 보존했습니다.",
      "mobile_rag_engine을 연동해 업로드된 약관을 온디바이스 SQLite, HNSW, BM25 하이브리드 검색으로 처리했습니다.",
      "보험 약관 intent 후보를 기반으로 즉시, 검토, 참고 항목을 정렬하는 자동 분석 파이프라인을 설계했습니다.",
      "동의 기반 AI 요약 생성, 서버 readiness 확인, 클라이언트 세션 처리, 로컬 fallback 요약 흐름을 구축했습니다.",
    ],
    cardPresentation: {
      variant: "product-screenshot",
      thumbnailScreenIndex: 7,
      description:
        "pdfrx, mobile_rag_engine, 로컬 SQLite/HNSW/BM25 검색, 조항 랭킹, 동의 기반 AI 요약을 연결한 Flutter 앱입니다.",
      evidenceBadges: [
        "Flutter",
        "pdfrx",
        "mobile_rag_engine",
        "PDF highlights",
        "QA",
      ],
      highlight:
        "인제스션 단계에서 페이지 좌표를 보존해 검색·분석 결과가 정확한 원문 하이라이트로 돌아가게 했습니다.",
    },
    techStack: [
      "Flutter",
      "Dart",
      "Riverpod",
      "GoRouter",
      "pdfrx",
      "mobile_rag_engine",
    ],
    color: "from-sky-500 to-emerald-500",
    iconType: "layers",
    screens: [
      {
        id: "00-home-dashboard",
        imageAlt: "Easy Contract Viewer 홈 대시보드",
        title: "홈 대시보드",
        desc: "업로드된 약관별 검토 현황과 남은 미검토 항목 수를 보여줍니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/00-home-dashboard.png",
      },
      {
        id: "01-analysis-overview",
        imageAlt: "Easy Contract Viewer 분석 결과 개요",
        title: "분석 결과 개요",
        desc: "약관 분석 후 즉시, 검토, 참고 항목을 요약해 보여줍니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/01-analysis-overview.png",
      },
      {
        id: "02-analysis-review-list-search-entry",
        imageAlt:
          "Easy Contract Viewer 검토 목록과 검색 진입",
        title: "검토 목록과 검색 진입",
        desc: "검토 카드와 약관 검색을 위한 Smart Guide 진입점을 함께 제공합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/02-analysis-review-list-search-entry.png",
      },
      {
        id: "03-analysis-review-item-expanded",
        imageAlt: "Easy Contract Viewer 검토 항목 확장",
        title: "검토 항목 확장",
        desc: "검토 항목을 펼쳐 매칭 조항, 근거, 다음 행동을 확인합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/03-analysis-review-item-expanded.png",
      },
      {
        id: "04-review-detail-highlight-summary",
        imageAlt:
          "Easy Contract Viewer 하이라이트 요약",
        title: "하이라이트 요약",
        desc: "원문 조항을 열기 전 주요 키워드와 확인 사항을 정리합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/04-review-detail-highlight-summary.png",
      },
      {
        id: "05-ai-summary-consent-dialog",
        imageAlt: "Easy Contract Viewer AI 요약 동의",
        title: "AI 요약 동의",
        desc: "AI 요약 생성 전 동의 항목과 주의 문구를 표시합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/05-ai-summary-consent-dialog.png",
      },
      {
        id: "06-ai-summary-generated",
        imageAlt: "Easy Contract Viewer 생성된 AI 요약",
        title: "생성된 AI 요약",
        desc: "원문 약관을 기준으로 확인해야 한다는 안내와 함께 간결한 AI 요약을 제공합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/06-ai-summary-generated.png",
      },
      {
        id: "07-pdf-source-highlight",
        imageAlt:
          "Easy Contract Viewer PDF 원문 하이라이트",
        title: "PDF 원문 하이라이트",
        desc: "매칭된 조항 위치의 PDF 원문을 열고 근거 영역을 하이라이트합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/07-pdf-source-highlight.png",
      },
      {
        id: "08-pdf-highlight-detail-sheet",
        imageAlt:
          "Easy Contract Viewer 하이라이트 상세 시트",
        title: "하이라이트 상세 시트",
        desc: "선택한 PDF 하이라이트의 조항 상세 내용을 표시합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/08-pdf-highlight-detail-sheet.png",
      },
      {
        id: "09-smart-guide-entry-card",
        imageAlt: "Easy Contract Viewer Smart Guide 진입 카드",
        title: "Smart Guide 진입 카드",
        desc: "분석 결과에서 뽑은 추천 키워드로 약관 검색을 시작합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/09-smart-guide-entry-card.png",
      },
      {
        id: "10-smart-guide-search-empty",
        imageAlt: "Easy Contract Viewer Smart Guide 검색 대기",
        title: "Smart Guide 검색 대기",
        desc: "검색어 입력 전 검색창과 빠른 태그를 제공합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/10-smart-guide-search-empty.png",
      },
      {
        id: "11-smart-guide-search-results",
        imageAlt: "Easy Contract Viewer Smart Guide 검색 결과",
        title: "Smart Guide 검색 결과",
        desc: "검색어와 일치하는 약관 조항을 하이라이트와 함께 보여줍니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/11-smart-guide-search-results.png",
      },
      {
        id: "12-smart-guide-result-pdf-highlight",
        imageAlt:
          "Easy Contract Viewer 검색 결과 PDF 하이라이트",
        title: "검색 결과 PDF 하이라이트",
        desc: "Smart Guide 검색 결과를 정확한 PDF 근거 위치로 연결합니다.",
        imagePath:
          "/images/easy-contract-viewer/android-screens/12-smart-guide-result-pdf-highlight.png",
      },
    ],
  },
  {
    id: "fiet-fitness-trainer",
    type: "tablet",
    title: "피에트 피트니스 트레이너",
    subtitle: "피에트 피트니스 센터의 트레이너 전용 앱",
    description:
      "피에트 피트니스 센터의 트레이너 전용 앱입니다. 회원 관리, 운동 프로그램 설계, 식단 관리 및 체중 데이터 추적을 태블릿에 최적화된 인터페이스로 제공합니다.",
    implementationPoints: [
      "트레이너 업무 흐름에 맞춘 태블릿 우선 Flutter UI와 재사용 위젯을 구축했습니다.",
      "체중계와 가속도 센서 연동을 위한 BLE 실시간 데이터 전송 흐름을 구현했습니다.",
      "Fastlane과 GitHub Actions로 모바일 배포 자동화 파이프라인을 구축했습니다.",
    ],
    techStack: ["Flutter", "Dart", "Firebase"],
    color: "from-indigo-500 to-purple-500",
    iconType: "dumbbell",
    screens: [
      {
        id: "splash",
        imageAlt:
          "피에트 피트니스 트레이너 Splash",
        title: "Splash",
        desc: "앱 시작 화면입니다.",
        imagePath: "/images/fiet-fitness-trainer/Splash.png",
      },
      {
        id: "report-inbody",
        imageAlt:
          "피에트 피트니스 트레이너 결과 리포트 - 인바디",
        title: "결과 리포트 - 인바디",
        desc: "회원의 인바디 측정 결과를 분석하여 표시합니다.",
        imagePath: "/images/fiet-fitness-trainer/report-inbody.png",
        scrollable: true,
      },
      {
        id: "report-fiet",
        imageAlt:
          "피에트 피트니스 트레이너 결과 리포트 - 피에트",
        title: "결과 리포트 - 피에트",
        desc: "피에트 피트니스 종합 분석 리포트를 제공합니다.",
        imagePath: "/images/fiet-fitness-trainer/report-fiet.png",
        scrollable: true,
      },
      {
        id: "video-recording",
        imageAlt:
          "피에트 피트니스 트레이너 동영상 촬영",
        title: "동영상 촬영",
        desc: "운동 자세 촬영 중인 화면입니다.",
        imagePath: "/images/fiet-fitness-trainer/video-recording.png",
      },
      {
        id: "video-vertical",
        imageAlt:
          "피에트 피트니스 트레이너 동영상 보기",
        title: "동영상 보기",
        desc: "촬영된 동영상을 세로로 확인합니다.",
        imagePath: "/images/fiet-fitness-trainer/video-vertical.png",
      },
      {
        id: "photo-captured",
        imageAlt:
          "피에트 피트니스 트레이너 사진 촬영",
        title: "사진 촬영",
        desc: "촬영된 사진을 확인하는 화면입니다.",
        imagePath: "/images/fiet-fitness-trainer/photo-captured.png",
      },
    ],
  },
  {
    id: "fiet-fitness-user",
    type: "mobile",
    title: "피에트 피트니스",
    subtitle: "AI 건강 관리 회사가 만든 새로운 개념의 피트니스 센터",
    description:
      "AI 건강 관리 회사가 만든 새로운 개념의 피트니스 센터 앱입니다. 트레이너와 연동하여 맞춤형 운동 프로그램과 식단을 확인하고 진행 상황을 기록합니다.",
    implementationPoints: [
      "Flutter 상태 기반 UI로 운동/식단 추적 화면을 구현했습니다.",
      "Firebase Cloud Messaging 기반 푸시 알림 워크플로를 연동했습니다.",
      "수분 섭취와 체중 변화 이력을 차트 중심 화면으로 시각화했습니다.",
    ],
    techStack: ["Flutter", "Dart", "Firebase"],
    color: "from-violet-500 to-fuchsia-500",
    iconType: "dumbbell",
    screens: [
      {
        id: "splash",
        imageAlt: "피에트 피트니스 Splash",
        title: "Splash",
        desc: "앱 시작 화면입니다.",
        imagePath: "/images/fiet-fitness-user/Splash.png",
      },
      {
        id: "record",
        imageAlt:
          "피에트 피트니스 기록하기",
        title: "기록하기",
        desc: "운동 및 활동을 기록하는 화면입니다.",
        imagePath: "/images/fiet-fitness-user/record.png",
      },
      {
        id: "water",
        imageAlt:
          "피에트 피트니스 수분 섭취",
        title: "수분 섭취",
        desc: "일일 수분 섭취량을 기록하고 확인합니다.",
        imagePath: "/images/fiet-fitness-user/water.png",
        scrollable: true,
      },
      {
        id: "diet",
        imageAlt: "피에트 피트니스 식단",
        title: "식단",
        desc: "식단 정보를 확인하고 관리합니다.",
        imagePath: "/images/fiet-fitness-user/diet.png",
        scrollable: true,
      },
      {
        id: "notification",
        imageAlt: "피에트 피트니스 알림",
        title: "알림",
        desc: "앱 알림을 확인하는 화면입니다.",
        imagePath: "/images/fiet-fitness-user/notification.png",
        scrollable: true,
      },
      {
        id: "weight",
        imageAlt: "피에트 피트니스 체중",
        title: "체중",
        desc: "체중 변화를 추적하고 그래프로 확인합니다.",
        imagePath: "/images/fiet-fitness-user/weight.png",
        scrollable: true,
      },
    ],
  },
  {
    id: "local-mobile-rag-gemma",
    type: "package",
    title: "mobile_rag_engine",
    releaseLabel: "pub.dev 0.18.6",
    subtitle: "모바일에서 완전 로컬 RAG를 실행하는 Flutter 패키지",
    description:
      "Flutter 앱에 서버 업로드 없는 온디바이스 문서 검색과 RAG 기능을 붙일 수 있는 공개 Flutter 패키지입니다.",
    implementationPoints: [
      "Flutter 공개 API, 예제, 문서, 릴리스 패키징을 포함해 mobile_rag_engine 0.18.6을 배포했습니다.",
      "제품 API 계층과 네이티브 hot path를 분리해 Flutter facade, Dart orchestration, Rust FFI core로 구성했습니다.",
      "문서 파싱, 청크 메타데이터, ONNX 임베딩, SQLite 저장, 색인 write까지 인제스트 경로를 구현했습니다.",
      "질의 임베딩, HNSW 벡터 검색, BM25 sparse 검색, 후보 결합, context assembly까지 쿼리 경로를 구현했습니다.",
      "Easy Contract Viewer 같은 제품에서 로컬 색인과 원문 기반 context 생성을 재사용할 수 있는 기반 기술로 구성했습니다.",
    ],
    cardPresentation: {
      variant: "architecture",
      thumbnailScreenIndex: 1,
      description:
        "Flutter facade, Dart orchestration, Rust FFI core, ONNX runtime, SQLite store, HNSW/BM25 검색 경로로 나뉜 온디바이스 RAG 패키지 아키텍처입니다.",
      evidenceBadges: [
        "Flutter API",
        "Dart orchestration",
        "Rust FFI",
        "ONNX",
        "SQLite",
        "HNSW/BM25",
      ],
      highlight:
        "Flutter API, Dart orchestration, Rust FFI 검색 코어, ONNX 임베딩, SQLite 저장소, HNSW/BM25 검색 경로로 구성했습니다.",
    },
    techStack: [
      "Flutter",
      "Dart",
      "Rust FFI",
      "ONNX Runtime",
      "SQLite",
      "HNSW/BM25",
    ],
    evidenceBadges: ["pub.dev", "GitHub", "Rust FFI", "ONNX", "HNSW/BM25"],
    color: "from-blue-500 to-cyan-400",
    iconType: "brain",
    screens: [
      {
        id: "overview",
        imageAlt: "mobile_rag_engine 패키지 개요",
        title: "패키지 개요",
        desc: "mobile_rag_engine은 Flutter 앱 안에서 로컬 문서 검색, retrieval, LLM-ready context 생성을 처리하는 pub.dev 공개 패키지입니다.",
        imagePath: "/images/mobile-rag-engine/problem.ko.svg",
      },
      {
        id: "architecture",
        imageAlt: "mobile_rag_engine 아키텍처",
        title: "아키텍처",
        desc: "패키지는 Flutter facade, Dart orchestration, Rust FFI search core, ONNX embedding runtime, SQLite chunk/index store, HNSW/BM25 retrieval path로 나뉩니다.",
        imagePath: "/images/mobile-rag-engine/architecture.ko.svg",
      },
      {
        id: "retrieval-flows",
        imageAlt: "mobile_rag_engine 연동 흐름",
        title: "연동 흐름",
        desc: "앱이 문서를 로컬에서 인제스트하고, chunk/index를 저장하고, HNSW/BM25 검색을 실행한 뒤 제품 기능에 근거 context를 넘길 수 있게 합니다.",
        imagePath: "/images/mobile-rag-engine/retrieval-flows.ko.svg",
      },
    ],
    links: [
      {
        label: "GitHub",
        url: "https://github.com/dev07060/mobile_rag_engine",
      },
      {
        label: "pub.dev",
        url: "https://pub.dev/packages/mobile_rag_engine",
      },
    ],
  },
  {
    id: "motgo",
    type: "web",
    title: "Motgo\n맛집 투표 서비스",
    subtitle: "친구들과 함께 찾은 맛집을 투표하고 공유하는 서비스",
    description:
      "네이버 지도 URL을 붙여넣으면 맛집 정보를 자동으로 불러오고, 친구들과 함께 투표할 수 있는 웹 서비스입니다. 실시간 투표 현황과 결과 분석 기능을 제공합니다.",
    implementationPoints: [
      "Next.js와 TypeScript로 실시간 투표 흐름과 공유 링크 기능을 구현했습니다.",
      "네이버 지도 URL에서 맛집 메타데이터를 자동 수집하는 파이프라인을 구축했습니다.",
      "Supabase(PostgreSQL)로 투표 데이터를 관리하고 Playwright로 E2E 검증을 구성했습니다.",
    ],
    techStack: [
      "Next.js",
      "TypeScript",
      "Tailwind CSS",
      "PostgreSQL",
      "Supabase",
      "Playwright",
    ],
    color: "from-purple-500 to-pink-500",
    iconType: "utensils",
    screens: [
      {
        id: "first-main-screen",
        imageAlt:
          "Motgo\n맛집 투표 서비스 메인 화면",
        title: "메인 화면",
        desc: "새로운 투표를 시작하거나 기존 투표에 참여할 수 있는 메인 화면입니다.",
        imagePath: "/images/motgo/first-main-screen.png",
      },
      {
        id: "copy-sharing-url",
        imageAlt:
          "Motgo\n맛집 투표 서비스 공유 링크 복사",
        title: "공유 링크 복사",
        desc: "친구들과 공유할 수 있는 투표 링크를 복사합니다.",
        imagePath: "/images/motgo/copy-sharing-url.png",
      },
      {
        id: "set-urls",
        imageAlt:
          "Motgo\n맛집 투표 서비스 URL 입력",
        title: "URL 입력",
        desc: "네이버 지도 URL을 입력하여 맛집 후보를 추가합니다.",
        imagePath: "/images/motgo/set-urls.png",
      },
      {
        id: "call-restaurant-infos",
        imageAlt:
          "Motgo\n맛집 투표 서비스 맛집 정보 불러오기",
        title: "맛집 정보 불러오기",
        desc: "입력한 URL에서 맛집 정보를 자동으로 스크래핑합니다.",
        imagePath: "/images/motgo/call-restaurant-infos.png",
      },
      {
        id: "sharing-vote-link",
        imageAlt:
          "Motgo\n맛집 투표 서비스 링크 공유",
        title: "링크 공유",
        desc: "카카오톡 등으로 투표 링크를 친구들에게 공유합니다.",
        imagePath: "/images/motgo/sharing-vote-link.png",
      },
      {
        id: "vote-page",
        imageAlt:
          "Motgo\n맛집 투표 서비스 투표 페이지",
        title: "투표 페이지",
        desc: "맛집 후보들을 확인하고 원하는 곳에 투표합니다.",
        imagePath: "/images/motgo/vote-page.png",
      },
      {
        id: "end-vote",
        imageAlt:
          "Motgo\n맛집 투표 서비스 투표 종료",
        title: "투표 종료",
        desc: "투표를 마감하고 결과를 확인합니다.",
        imagePath: "/images/motgo/end-vote.png",
      },
      {
        id: "result-page-1",
        imageAlt:
          "Motgo\n맛집 투표 서비스 결과 페이지 1",
        title: "결과 페이지 1",
        desc: "투표 결과를 순위별로 확인할 수 있습니다.",
        imagePath: "/images/motgo/result-page-1.png",
      },
      {
        id: "result-page-2",
        imageAlt:
          "Motgo\n맛집 투표 서비스 결과 페이지 2",
        title: "결과 페이지 2",
        desc: "상세한 투표 통계와 분석 결과를 제공합니다.",
        imagePath: "/images/motgo/result-page-2.png",
      },
    ],
  },
  {
    id: "haru-check",
    type: "mobile",
    title: "HaruCheck",
    subtitle: "AI 기반 건강 습관 추적 및 분석 서비스",
    description:
      "일일 운동/식단 인증을 기록하고, Google VertexAI (Gemini)를 활용한 AI 주간 리포트를 제공하는 건강 관리 앱입니다. 캘린더 기반 추적과 인터랙티브 데이터 시각화를 지원합니다.",
    implementationPoints: [
      "Flutter + Riverpod 상태 아키텍처로 인증 기록 UX를 구현했습니다.",
      "Cloud Functions와 Vertex AI(Gemini) 연동으로 주간 AI 리포트 생성 파이프라인을 구축했습니다.",
      "캘린더 히스토리와 소셜 피드 기능을 위한 Firebase 데이터 모델을 설계했습니다.",
    ],
    techStack: [
      "Flutter",
      "Firebase",
      "Firestore",
      "VertexAI",
      "Riverpod",
      "Cloud Functions",
    ],
    color: "from-orange-500 to-red-400",
    iconType: "activity",
    screens: [
      {
        id: "add-cert-1",
        imageAlt: "HaruCheck 인증 추가 1",
        title: "인증 추가 1",
        desc: "운동 또는 식단 인증을 사진과 함께 기록합니다.",
        imagePath: "/images/haru-check/add_cert_1.png",
      },
      {
        id: "add-cert-2",
        imageAlt: "HaruCheck 인증 추가 2",
        title: "인증 추가 2",
        desc: "인증 상세 정보를 입력합니다.",
        imagePath: "/images/haru-check/add_cert_2.png",
      },
      {
        id: "certs",
        imageAlt: "HaruCheck 인증 목록",
        title: "인증 목록",
        desc: "캘린더 기반으로 날짜별 인증 기록을 확인합니다.",
        imagePath: "/images/haru-check/certs.png",
      },
      {
        id: "cert-detail",
        imageAlt: "HaruCheck 인증 상세",
        title: "인증 상세",
        desc: "개별 인증의 상세 정보를 확인합니다.",
        imagePath: "/images/haru-check/cert_detail.png",
      },
      {
        id: "load-report",
        imageAlt: "HaruCheck 리포트 로딩",
        title: "리포트 로딩",
        desc: "AI가 주간 데이터를 분석하여 리포트를 생성합니다.",
        imagePath: "/images/haru-check/load_report.png",
      },
      {
        id: "report-1",
        imageAlt: "HaruCheck AI 리포트 1",
        title: "AI 리포트 1",
        desc: "운동 빈도와 카테고리 분포를 차트로 시각화합니다.",
        imagePath: "/images/haru-check/report_1.png",
      },
      {
        id: "report-2",
        imageAlt: "HaruCheck AI 리포트 2",
        title: "AI 리포트 2",
        desc: "주간 활동 패턴을 분석합니다.",
        imagePath: "/images/haru-check/report_2.png",
      },
      {
        id: "report-3",
        imageAlt: "HaruCheck AI 리포트 3",
        title: "AI 리포트 3",
        desc: "영양 균형 분석 결과를 제공합니다.",
        imagePath: "/images/haru-check/report_3.png",
      },
      {
        id: "report-4",
        imageAlt: "HaruCheck AI 리포트 4",
        title: "AI 리포트 4",
        desc: "강점과 개선점을 포함한 맞춤형 피드백을 제공합니다.",
        imagePath: "/images/haru-check/report_4.png",
      },
      {
        id: "report-5",
        imageAlt: "HaruCheck AI 리포트 5",
        title: "AI 리포트 5",
        desc: "다음 주 목표 설정을 위한 제안을 제공합니다.",
        imagePath: "/images/haru-check/report_5.png",
      },
      {
        id: "user-list",
        imageAlt: "HaruCheck 사용자 목록",
        title: "사용자 목록",
        desc: "다른 사용자들의 인증 기록을 확인할 수 있는 소셜 피드입니다.",
        imagePath: "/images/haru-check/user_list.png",
      },
    ],
    links: [
      {
        label: "서비스 보기",
        url: "https://seol-haru-check.web.app",
      },
      {
        label: "GitHub",
        url: "https://github.com/dev07060/seol_haru_check",
      },
    ],
  },
  {
    id: "weedool",
    type: "mobile",
    title: "Weedool (TuringBio)",
    subtitle: "BA 활용 디지털 치료 서비스",
    description:
      "BA(행동 활성화) 기법을 활용한 디지털 치료 지향 서비스입니다. 감정 선택, BA 추천, 상태 모니터링, 심리 상담 센터 소개 등의 기능을 제공합니다.",
    implementationPoints: [
      "Flutter + Riverpod + GoRouter로 상담/활동 플로우를 구현했습니다.",
      "Naver Map API와 분석 이벤트를 연동해 상담센터 탐색 흐름을 구성했습니다.",
      "Google Analytics 기반 행동 데이터 추적으로 사용자 상호작용 지표를 개선했습니다.",
    ],
    techStack: [
      "Flutter",
      "Riverpod",
      "GoRouter",
      "Naver Map API",
      "Google Analytics",
    ],
    color: "from-teal-500 to-green-400",
    iconType: "heart",
    screens: [
      {
        id: "unnamed-2",
        imageAlt:
          "Weedool (TuringBio) 맞춤형 AI 마음관리",
        title: "맞춤형 AI 마음관리",
        desc: "일상에서 함께하는 맞춤형 AI 마음관리 서비스, 위들라이프입니다.",
        imagePath: "/images/weedool/unnamed-2.png",
      },
      {
        id: "unnamed-3",
        imageAlt:
          "Weedool (TuringBio) 활동으로 마음 회복",
        title: "활동으로 마음 회복",
        desc: "임상적 실험과 논문을 통해 검증된 활동 추천 프로그램을 제공합니다.",
        imagePath: "/images/weedool/unnamed-3.png",
      },
      {
        id: "unnamed-4",
        imageAlt:
          "Weedool (TuringBio) AI 맞춤형 활동 루틴",
        title: "AI 맞춤형 활동 루틴",
        desc: "분석 결과를 바탕으로 AI가 나만의 활동을 제안해요.",
        imagePath: "/images/weedool/unnamed-4.png",
      },
      {
        id: "unnamed-5",
        imageAlt: "Weedool (TuringBio) 활동 후 기록",
        title: "활동 후 기록",
        desc: "나의 변화와 하루를 자연스럽게 기록하고, 사진과 소감을 남겨보세요.",
        imagePath: "/images/weedool/unnamed-5.png",
      },
      {
        id: "unnamed-6",
        imageAlt: "Weedool (TuringBio) AI와 대화",
        title: "AI와 대화",
        desc: "언제든 편안하게 감정을 털어놓을 수 있어요. AI와 대화하며 마음을 나눠요.",
        imagePath: "/images/weedool/unnamed-6.png",
      },
      {
        id: "unnamed-7",
        imageAlt:
          "Weedool (TuringBio) 감정/행동 리포트",
        title: "감정/행동 리포트",
        desc: "내 마음이 어떻게 달라졌는지 감정과 행동의 변화를 리포트로 확인해요.",
        imagePath: "/images/weedool/unnamed-7.png",
      },
    ],
    links: [
      {
        label: "Google Play Store",
        url: "https://play.google.com/store/apps/details?id=com.turingbio.trb_weedool_school_app",
      },
      {
        label: "iOS App Store",
        url: "https://apps.apple.com/kr/app/%EC%9C%84%EB%91%98-weedool/id6744308822",
      },
    ],
  },
];
